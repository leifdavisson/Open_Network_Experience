...
def test_01_empty_fleet_zero_state_truth_invariants():
    ...
    assert dash_html_path.exists()

    with open(dash_html_path, "r", encoding="utf-8") as f:
        html_content = f.read()

    # Invariants: Zero-state HTML defaults MUST NOT show hardcoded live stats
    assert '<div class="metric-value" id="cb-kpi-rssi">-- dBm</div>' in html_content
    assert '<div class="metric-value" id="cb-kpi-mos">-- / 5.0</div>' in html_content
    assert '<div class="metric-value" id="cb-kpi-sla">--%</div>' in html_content
    assert 'id="cb-kpi-rssi-sub">No Active Stream<' in html_content
    assert 'id="cb-kpi-mos-sub">No Active Sessions<' in html_content
...
def test_02_offline_sensor_data_masking_in_schema_and_ui():
    ...
    assert offline_item.is_online is False
    assert offline_item.sensor_id == "cb-offline-test"
    ...
    assert "const isOnline = Boolean(cb.is_online);" in js_code
    assert "`<span style=\"color:var(--text-muted);\">-- (Offline)</span>`" in js_code
    assert "`<span style=\"color:var(--text-muted);\">-- (N/A)</span>`" in js_code
    assert "`<span style=\"color:var(--text-muted);\">Disconnected</span>" in js_code
...
def test_03_database_to_api_schema_fidelity():
    ...
    assert safe_resp.sensor_id == raw_db_record["sensor_id"]
    assert safe_resp.location.district == "Unified School District"
    assert safe_resp.location.room == "IT Operations"
...
def test_06_data_truthfulness_oracle_matrix(is_online, rssi, mos, expected_kpi_sub):
    ...
    assert kpi_sub == expected_kpi_sub
...
def test_07_modal_offline_stale_warning_banner():
    ...
    assert "⚠️ Device Offline — Stale Telemetry Warning" in js_code
    assert "Metrics below represent a frozen snapshot and are not live." in js_code
    assert "formatTimeAgo" in js_code

    # Assert live streaming stream indicator is rendered when online
    assert "● Live Telemetry Stream Active" in js_code

    # Assert that no fake CAASPP/Google/Clever fallbacks are hardcoded in the modal
    assert "CAASPP Testing: 45ms" not in js_code
    assert "Google Classroom: 32ms" not in js_code
    assert "Clever SSO: 28ms" not in js_code
...
def test_08_slide_level_fleet_offline_warning_banner():
    ...
    assert '<div id="cb-fleet-offline-banner"></div>' in html_code
    assert "⚠️ Chromebook Fleet Offline" in html_code
    assert "No active telemetry streams detected. Live KPI metrics are paused" in html_code
...
def test_09_dedicated_chromebook_fleet_view_and_lock_controls():
    ...
    assert 'id="nav-manage-chromebooks"' in html_code
    assert "Chromebook Fleet" in html_code

    # Dedicated view container verification
    assert '<div class="view-section" id="view-manage-chromebooks">' in html_code
    assert 'id="fleet-lock-status-badge"' in html_code
    assert 'id="btn-fleet-lock"' in html_code
    assert 'id="btn-fleet-unlock"' in html_code
    assert 'id="fleet-helpdesk-pin"' in html_code
    assert 'id="cb-dedicated-fleet-table-body"' in html_code

    # Direct download links for Google Workspace staging
    assert '/api/v1/chromebooks/download/extension.zip' in html_code
    assert '/api/v1/chromebooks/download/policy.json' in html_code
